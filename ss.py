...
class KeyboardListener:
    def __init__(self, shared_state: SharedState):
        ...
        self.double_press_threshold = 0.2  # Reduced from 0.3 to 0.2 seconds
    ...
    def handle_delayed_single_press(self, char):
        if self.manual_input_buffer == char:  # If buffer hasn't been cleared by a double press
...
def process_section(shared_state: SharedState, json_file_path: str, chapter_index: int, section_index: int):
    ...
    print(f"\nProcessing Section: '{section_name}' in Chapter: '{chapter_name}'")
...
def cleanup_and_quit(shared_state: SharedState, json_file_path: str, keyboard_listener: KeyboardListener):
    ...
    with shared_state.lock:
        ...
        if prev_section_index >= 0:
            ...
            thread.join(timeout=20)  # Wait up to 10 seconds
            if thread.is_alive():
...
def handle_section_creation(shared_state: SharedState, keyboard_listener: KeyboardListener, json_file_path: str):
    ...
    print(f"\nMoved to New Section: '{section_name}'")
...
def handle_chapter_creation(shared_state: SharedState, keyboard_listener: KeyboardListener, json_file_path: str):
    ...
    print(f"\nMoved to New Chapter: '{chapter_name}'\nCurrent Section: '{section_name}'")
...
def main():
    ...
    if choice == 'new':
        ...
        print(f"\nStarted New Chapter: '{chapter_name}'\nCurrent Section: '{section_name}'")
        ...
        with shared_state.lock:
            ...
            print(f"\nResuming from Chapter: '{chapter['chapter_name']}'\nCurrent Section: '{section['section_name']}'")
    else:
